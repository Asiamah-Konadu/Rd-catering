...
export default function CartPage() {
  ...
  return <main className="page narrow"><div className="page-heading"><span className="eyebrow">Your order</span><h1>Shopping bag</h1></div>
    ...
    <div className="cart-layout"><div className="cart-list">{items.map(x => <div className="cart-row" key={x.id}><div><strong>{x.name}</strong><span>GH₵ {x.price.toFixed(2)} each</span></div><div className="qty"><button onClick={() => setQuantity(x.id,x.quantity-1)}><Minus size={15}/></button><b>{x.quantity}</b><button onClick={() => setQuantity(x.id,x.quantity+1)}><Plus size={15}/></button></div><strong>GH₵ {(x.price*x.quantity).toFixed(2)}</strong><button className="icon-btn" onClick={() => remove(x.id)}><Trash2 size={17}/></button></div>)}</div>
